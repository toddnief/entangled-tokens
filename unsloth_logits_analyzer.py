#!/usr/bin/env python3
"""
Unsloth-Optimized Logits Analyzer for A100
Ultra-fast full vocabulary logprobs using Unsloth optimizations
"""
...
class UnslothTokenAnalysis:
    ...
    position: int
    rank: int
    full_logprobs: Optional[np.ndarray] = None
    top_k_alternatives: Optional[List[Dict]] = None


class UnslothLogitsAnalyzer:
    """Ultra-fast analyzer using Unsloth optimizations for full vocabulary logprobs"""
    ...
    def __init__(self, 
        ...
        """
        Initialize Unsloth-optimized analyzer
        ...
        self.output_dir = Path(self.config.get('output_dir', 'unsloth_logits_outputs'))
        self.compress = self.config.get('compress', True)
        
        print(f"Initializing UnslothLogitsAnalyzer:")
    ...
    def _load_unsloth_model(self):
        ...
        try:
            ...
            if hasattr(self.model.config, 'use_flash_attention_2'):
                print(f"  Flash Attention 2: {self.model.config.use_flash_attention_2}")
    ...
    def get_full_logits(self, text: str, return_full_vocab: bool = False, top_k: int = 1000) -> Dict[str, Any]:
        """
        Get complete logits for every token position with Unsloth speed optimizations
    ...
    def analyze_openwebtext(self, 
        ...
        """
        Analyze OpenWebText dataset with Unsloth speed optimizations
        """
        ...
        print(f"Starting Unsloth-optimized OpenWebText analysis...")
        print(f"  Dataset: {dataset_name}")
        print(f"  Samples: {num_samples}")
        print(f"  Chunk size: {chunk_size}")
        print(f"  Top-k alternatives: {top_k}")
        print(f"  Full vocab: {return_full_vocab}")
        print(f"  🚀 Unsloth optimizations enabled: {UNSLOTH_AVAILABLE}")
        ...
        run_id = f"{model_short_name}_unsloth_{timestamp}"
        ...
        try:
            ...
            for i, example in enumerate(tqdm(dataset.take(num_samples), desc="Processing samples", total=num_samples)):
                ...
                if should_save:
                    ...
                    if i % 10 == 0:
                        ...
                        print(f"  📊 Batch {file_counter}: {len(current_batch)} samples, {samples_per_sec:.2f} samples/sec")
            ...
            print(f"❌ Error in Unsloth analysis: {e}")
            logging.error(f"Unsloth analysis error: {e}", exc_info=True)
            return []
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Unsloth-optimized logits analyzer for A100")
    ...
    if args.config:
        ...
        analyzer = UnslothLogitsAnalyzer.from_config(args.config, **overrides)
    else:
        # Use command line arguments
        analyzer = UnslothLogitsAnalyzer(
    ...
    if args.openwebtext:
        # Analyze OpenWebText with Unsloth speed
        results = analyzer.analyze_openwebtext(
            num_samples=args.num_samples,
            return_full_vocab=args.full_vocab,
            top_k=args.top_k
        )
        print(f"\n🚀 Completed Unsloth analysis of {len(results)} samples")
        ...
        print("Example: python unsloth_logits_analyzer.py --openwebtext --num-samples 100")
